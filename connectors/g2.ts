...
export default class G2Connector extends ConnectorRuntime {
  ...
  async sync(ctx: SyncContext): Promise<SyncResult> {
    ...
    // Extract product key from URL for external_id generation
    const productMatch = productUrl.match(/\/products\/([^/]+)/);
    const productKey = productMatch ? productMatch[1] : 'unknown';

    const baseUrl = productUrl;
    const checkpoint = (ctx.checkpoint ?? {}) as G2Checkpoint;
    const allEvents: EventEnvelope[] = [];

    const { browser } = await launchBrowser({} as any, { stealth: true });
      ...
      for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
          ...
          reviewCards.forEach((card) => {
            try {
              ...
              const detailDivs = parentDiv
                ? Array.from(
                    parentDiv.querySelectorAll('.elv-text-subtle')
                  )
                : [];
        ...
        for (const review of reviews) {
          const event: EventEnvelope = {
            external_id: `g2-${productKey}-${review.date || 'nodate'}-${review.author.replace(/\s+/g, '-')}`,
            title: review.title,
            content: review.text,
